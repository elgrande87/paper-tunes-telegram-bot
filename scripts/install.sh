...
# Works from a clone and via: curl -fsSL .../install.sh | bash
...
if [[ "${PT_BOOTSTRAPPED:-0}" != "1" && "${PT_NO_PULL:-0}" != "1" ]]; then
  git pull --ff-only
fi
...
if grep -q '^PT_TELEGRAM_BOT_TOKEN=123456789:REPLACE_ME' "$ENV_FILE"; then
  ...
  echo "Token eintragen: nano $ENV_FILE"
  echo "Danach: sudo systemctl start $SERVICE_NAME"
else
